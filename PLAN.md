...
- [x] Buat model dan sinkronisasi `sales_orders_queue` dan `visit_logs_queue` untuk antrian pesan mobile.
- [x] Halaman "Inbox Sales Order" (desktop) - tampilkan `sales_orders_queue` status pending, edit qty/hapus item sebelum approve, approve = buat Nota baru (status SO) di database lokal desktop + update queue jadi APPROVED (atomik 1 transaksi).
- [x] Halaman "Inbox Kunjungan" (desktop) - tampilkan `visit_logs_queue` status pending, review foto, link Google Maps GPS, approve = buat `customers` baru jika `is_new_customer = true` + update queue jadi APPROVED.
- [x] Otorisasi perubahan status nota ke TEMPO/DONE dan pengamanan hashing password via PBKDF2/SHA256 (`PasswordHasherService`).
